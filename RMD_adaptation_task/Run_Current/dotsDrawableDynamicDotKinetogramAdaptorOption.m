classdef dotsDrawableDynamicDotKinetogramAdaptorOption < dotsDrawableVertices
    ...
    properties
       ...
       %***Kyra: For if the object is to be an adaptor or not
       adaptor = false;
       tPFS=0;   %Need to have set from configure for the specific trial. 
       tview=0;  %Need to get from logic via configure
       timer=false;  %Before start the stimulus, need to set the timer to true, probably in the exit part of the state machine
       finalswitched=false; %determines whether to do the final switch yet.
        shorty=0; %offset number
    ...
    properties (SetAccess = protected)
        ...
        j=0; %Random point in cycle to start
    ...
    methods
        ...
        function self = dotsDrawableDynamicDotKinetogramAdaptorOption()
        ...
        function prepareToDrawInWindow(self)
            ...
            %self.j=round(rand*60/self.H);
            %disp('frame H is');
            %disp(self.frameH);
            %disp('window Frame Rate is');
            %disp(self.windowFrameRate);
        ...
        function computeNextFrame(self)
            ...
            if self.timer %This portion makes sure that the switching happens for the correct amount of time
                tic;
                self.timer=false;
                self.j=0;
                if self.H==0.5
            ...
            if toc<self.tview  %***If the stimulus has not been visable for the full time of switching then:
           
            
            if mod((self.j),60/self.H)==0  %*** This is what is changing the direction! If the number of frames+offset (added above already) is a multiple of the period, switch directions
                %Modification:
                ...
                %Original:  self.direction = mod(self.direction+180,360);
            end
            
            % Otherwise if the final switch has not yet occured but the
            % time of viewing the pre-final is over, switch and say the
            % final switch has happened
            elseif toc>(self.tview) && ~self.finalswitched
                if self.direction==45;
                    self.direction=135;
                else
                    self.direction=45;
                end
                self.finalswitched=true;
                
                
                
            end
            self.j=self.j+1;  %%%Add a counter to the number of frames the stimulus has been on
            end
