...
%JLC preparation, color change in fpInd seem to just indicate at what stage the
%preperation is at
    ...
    %JLC if there is no adapt status, the adapt state will be skipped. If there
    %is the function in entry will change the adapt status to have
    %associated timeout
    ...
    %JLC actual stim displayed only for .5
    'stimulus1'  {on stimInd}   .5       {} 'stimulus0' {}; ...
    %getting read to record result (clear ui, set a time stamp)
    'stimulus0'  {@flushui, list}   0    {@setTimeStamp, logic}             'decision'     {}; ...
    %JLC getNextEvent records the user decision. Here the user is only given a
    %set amount of time to make a decision
    ...
    %JLC this state is never entered. Also closeTree is not defined
...
%JLC Todo: More adjustments to the drawlables? If this is not a trial by
%trial adjustment, remove and place somewhere where all drawlable
