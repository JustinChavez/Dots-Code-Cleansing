...
stim = dotsDrawableDynamicDotKinetogramAdaptorOption();
...
adaptr=dotsDrawableDynamicDotKinetogramAdaptorOption();
adaptr.colors = list{'graphics'}{'lightblue'};
adaptr.pixelSize = 5; % size of the dots
adaptr.direction = 0;                         %Maybe switch to 45?
adaptr.density = 35;
...
adaptl=dotsDrawableDynamicDotKinetogramAdaptorOption();
adaptl.colors = list{'graphics'}{'lightgray'};
adaptl.pixelSize = 5; % size of the dots
adaptl.direction = 0;                         %Maybe switch to 45?
adaptl.density = 35;
...
%   'inst'      {@doNextInstruction, av} 1        {}              ''; ...
    ...
    'check adapt status' {@checkAdaptStatus, trialStates, list} 0 {} 'change adaptor time' {};...  %default to not-adapting trials
    'adapt'     {on [adaptorR adaptorL]}   0   {off [adaptorR adaptorL]} 'change-time'  {};...          %time how long adaptors on for
    'change-time'      {@editState, trialStates, list, logic}   0    {@starttimer, list}    'stimulus1'     {}; ...  %*** Changed, @starttimer This is where you want to tell it to start the clock
...
drawables.setObjectProperty( ...
    'tPFS', logic.tPFS, [stimInd]);

drawables.setObjectProperty( ...
    'tview', logic.duration, [stimInd]);
...
function pickColor(list)
  drawables = list{'graphics'}{'drawables'};
    ...
    disp(colordir);
    if colordir==0
        a=list{'graphics'}{'lightgray'};
        b=list{'graphics'}{'lightblue'};
    else
        a=list{'graphics'}{'lightblue'};
        b=list{'graphics'}{'lightgray'};
    end
...
logic = list{'object'}{'logic'};
trialStates.editStateByName('stimulus1', 'timeout', logic.duration+logic.tPFS);
...
if logic.adaptor==1
    ...
    if mod(logic.adaptorCounter,10)==0
        trialStates.editStateByName('adapt', 'timeout', 7)   %real=25, demo=7     % Every fourth trial do 45 seconds of adapting stim
    else
        trialStates.editStateByName('adapt', 'timeout', 4) %real=12.5?
    end
...
function starttimer(list)
    ...
    logic = list{'object'}{'logic'};
     drawables.setObjectProperty( ...
    'shorty', logic.shorty, [stimInd])
    drawables.setObjectProperty( ...
    'timer', true, [stimInd])
    drawables.setObjectProperty( ...
    'finalswitched', false, [stimInd])
