function [dataFileName, taskPhase, id] = gatherTAFCDotsSubInfoQuest(tag, id, session)
...
taskPhase = phaseLabels{2};

% ask for the task phase- Kyra Skipped it
...
nameVetted = false;
while ~nameVetted
    dataFileName = fullfile('QuestTrials',sprintf('%s_%s_%s_%d',tag,id,taskPhase,session));
