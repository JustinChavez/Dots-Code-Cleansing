...
duration = NaN(1,dim1*dim2);
reac_time = NaN(1,dim1*dim2);
...
for i=1:dim1
   ...
   for j=1:dim2
       ...
       duration(index) = dur;
       reac_time(index) = all_data(i).statusData(j).ReactionTimeData;
...
save(strcat('H_', H,'_',coh,'/duration'),'duration');
save(strcat('H_', H,'_',coh,'/reac_time'),'reac_time');
